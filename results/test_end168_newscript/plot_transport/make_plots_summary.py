...
def plot_networks_together(networks, year):
    ...
    for network in networks:
        ...
        links = network.links[network.links.carrier.str.contains("land transport")]
        if links.empty:
             links = network.loads[network.loads.carrier.str.contains("land transport")]

        vehicles = list()

        for i in links.index:    
             "rename transport vehicle types"        
             ...
             if ind == "oil":
                  ...
                  print("unknown vehicle type:", ind)
                  ind = "unknown"
             ...
             if snakemake.config["sector"]["endogenous_transport"]:
                ...
                if ind != "unknown":
                    vehicles.append(network.loads_t.p[i].sum().sum()/snakemake.config["sector"][ind + '_consumption_1car'])
             if ind == 'EV':
                #print(vehicles)
